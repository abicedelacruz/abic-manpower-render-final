...
def add_employee():
    ...
    emp = Employee(name=name, client_id=client_id, username=username, password_hash=generate_password_hash(pwd), monthly_salary=monthly, rest_day=rest_day)
    db.session.add(emp); db.session.commit()
    flash(f'Created employee {name} with username: {username} and password: {pwd}', 'info')
    return redirect(url_for('admin_dashboard'))
